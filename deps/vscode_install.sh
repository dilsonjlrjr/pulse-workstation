...
function vcode_install_dep() {
    curl https://packages.microsoft.com/keys/microsoft.asc | gpg --dearmor > ${COMMON_TEMP_DIR}/microsoft.gpg
    sudo install -o root -g root -m 644 ${COMMON_TEMP_DIR}/microsoft.gpg /etc/apt/trusted.gpg.d/
